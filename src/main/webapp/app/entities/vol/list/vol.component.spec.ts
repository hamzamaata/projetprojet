...
describe('Vol Management Component', () => {
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      imports: [RouterTestingModule.withRoutes([{ path: 'vol', component: VolComponent }]), HttpClientTestingModule, VolComponent],
      providers: [
        {
  ...
  it('Should call load all on init', () => {
    ...
    expect(comp.vols?.[0]).toEqual(expect.objectContaining({ id: 123 }));
  });

  describe('trackId', () => {
    it('Should forward to volService', () => {
      const entity = { id: 123 };
      jest.spyOn(service, 'getVolIdentifier');
      const id = comp.trackId(0, entity);
      expect(service.getVolIdentifier).toHaveBeenCalledWith(entity);
